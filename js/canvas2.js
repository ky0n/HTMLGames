...
canStartY = document.getElementById("header").offsetHeight;
...
let spilledOutBubble = "rgba(150, 183, 198, 0.4)"; // blue

let col = [
    "rgba(151, 237, 160, 0.6)", // green
    "rgba(223, 129, 91, 0.6)", // orange - red
...
let Ball = function (r, x, y, dx, dy, color) {
    ...
    this.update = function () {
        ...
        if ((this.y + this.r) > innerHeight || (this.y - this.r) < canStartY) {
            this.dy = -this.dy;
        }
...
function randomNumFromRange(min, max) {
    ...
    if (random !== 0) {
        if (min < 0) {
            return min + random * (Math.abs(min) + max);
        } else {
...
function explode() {
    ...
    for (let j = 0; j < (countDiff); j++) {
        ...
        if (posy > innerHeight || posy < canStartY) {
            if (posy > innerHeight) {
                posy = innerHeight - 20;
                if (speedY > 0) {
                    speedY = -speedY;
        ...
        circles.push(new Ball(5 * randomNumFromRange(0.5, 2), posx, posy, speedX, speedY, spilledOutBubble));
        circles[j].draw();
    }
}
...
let mouseCircle = new Ball(MOUSE_START_RAD, can.width - 100, can.height - 100, 0, 0, spilledOutBubble);
let circles = [];
...
init = function () {
    ...
    for (let i = 0; i < CIRCLECOUNT; i++) {
        ...
        let y = Math.random() * (can.height - 10) + canStartY;
